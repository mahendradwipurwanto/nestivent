    ...
    <ul class="step step-icon-sm step-avatar-sm">
      <?php if ($aktivitas == FALSE): ?>
        ...
        <?php if ($c_aktivitas > 8): ?>
          <a class="btn btn-block btn-white" href="<?= site_url('k-panel/aktivitas-k-panel') ?>">Lihat semua <i class="tio-chevron-right"></i></a>
        <?php endif; ?>
      </div>
      <!-- End Body -->
    </div>
  ...
  $(".js-nav-tooltip-link").on("show.bs.tooltip", function(e) {
    if (!$("body").hasClass("navbar-vertical-aside-mini-mode")) {
      return false;
        ...
        // =======================================================
        $('#aksesCepatPopover').popover('show');

        $(document).on('click', '#closeAksesCepatPopover' , function() {
          $('#aksesCepatPopover').popover('dispose');
        });
